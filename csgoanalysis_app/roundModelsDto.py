...
class RoundPlayerDto(models.Model):
    ...
    @staticmethod
    def create(frame, player_type, player, id_, round_):
        ...
        player_dto.viewx = list(frame.values_list(player_type + '_viewx', flat=True))
...
class RoundDto(models.Model):
    ...
    def from_round(round_):
        ...
        round_dto.CTpredictions = list(range(2))
        for i, (player_type, id_) in enumerate(player_types):
            round_dto.players[i] = RoundPlayerDto.create(
                frame, player_type, frame.select_related(player_type).first().__getattribute__(player_type), id_, round_)
