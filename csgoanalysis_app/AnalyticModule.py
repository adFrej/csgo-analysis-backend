...
class AnalyticModule:
    ...
    def get_all(self, states, score, damages, kills):
        kills_prep = kills.loc[~kills.attackerID.isna(), ["roundNum", "attackerID"]].reset_index(drop=True)
        damages = damages[(damages["weapon"] != "C4") & (damages["weapon"] != "World")]
        transformed_data = self.transform_data(states, score, kills_prep).reset_index(drop=True)
        pred = self.get_predictions(states, transformed_data).reset_index(drop=True)
        scores = self.get_ratings(initial_data=states, transformed_data=transformed_data, prediction=pred,
                                  damages=damages, kills=kills).reset_index(drop=True)
        # acc_rating = self.get_accumulated_rating(scores).reset_index(drop=True)
        moments = self.find_important_moments(pred, states.roundNum).reset_index(drop=True)
        return pred["probaCtWin"], scores, moments["important_moments"]

    def transform_data(self, input_data, score, kills):
        corrected_data = self._correct_last_place(input_data).reset_index(drop=True)
        ...
        dist_bomb_cols = self._get_distance_to_bombsites(corrected_data).reset_index(drop=True)
        dist_players_cols = self._get_distance_players(corrected_data).reset_index(drop=True)
        spotters_cols = self._get_spotters(corrected_data).reset_index(drop=True)
        active_weap_cols = self._convert_active_weapons(corrected_data, 'activeWeapon').reset_index(drop=True)
        main_weap_cols = self._convert_weapons(corrected_data, 'mainWeapon').reset_index(drop=True)
        sec_weap_cols = self._convert_sec_weapons(corrected_data, 'secondaryWeapon').reset_index(drop=True)
        ...
        kills_cols = self._prepare_kills(corrected_data, kills).reset_index(drop=True)
        groupped_cols = self._get_grouped_players(corrected_data).reset_index(drop=True)
        conquer_cols = self._conquer_map(corrected_data).reset_index(drop=True)
        ...
        result = result.merge(score, left_on='roundNum', right_on='roundNum').reset_index(drop=True)
    ...
    def share_proba(self, actions):
        ...
        for action in actions:
            ...
            if action["prediction"] != action["next_prediction"] and action["attackerSide"] != action["victimSide"]:
    ...
    def get_accumulated_rating(self, rating):
        summed_rating = rating.groupby(["roundNum", "playerID"]).agg({"gainValue": "sum"}).reset_index()
        mean_over_rounds = summed_rating.groupby("playerID").agg({"gainValue": "mean"})
